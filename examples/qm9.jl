...
function (m::Toy)(t,Xt)
    ...
    locs = tensor(Xt[1])
    x =  l.d_encoder(tensor(Xt[2])) #+ l.loc_encoder(vcat(l.loc_rff(locs),locs))
...
P = CoalescentFlow((OUFlow(20f0, 4f0, 0.1f0, -2f0), Flowfusion.DistNoisyInterpolatingDiscreteFlow()), Beta(1,5)) #Extreme schedule.
...
iters = 50000
for i in 1:iters
    t = rand(Float32, 200) .* 0.999f0 .+ 0.0005f0 #To stop Gaussain combine errors - I should really write a shortcircuit for this...
    bat = branching_bridge(P, X0sampler, [X1target() for _ in 1:200], t, coalescence_factor = 1.0, merger = BranchingFlows.select_anchor_merge);
    ...
    l,g = Flux.withgradient(model) do m
        ...
        mse_loss = floss(P.P[1], X1hat[1], X1targets[1], scalefloss(P.P[1], t, 1, 0.2f0)) * 2
        ...
        splits_loss = floss(P, hat_splits, splits_target, padmask, scalefloss(P, t, 1, 0.2f0)) / 3
        if i % 10 == 0
...
X0 = BranchingFlows.BranchingState(BranchingFlows.regroup([[X0sampler(nothing) for _ in 1:2]]), [1:2 ;;]) #Note: You MUST get the batch dimension back in. The model will need it, and the sampler assumes it.
paths = Tracker()
