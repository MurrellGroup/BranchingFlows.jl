...
if get(ENV, "FLOWCEPTION_DEMO_SKIP_PKG", "false") != "true"
...
use_gpu = get(ENV, "FLOWCEPTION_DEMO_NO_CUDA", "false") == "false"
if use_gpu
    Pkg.add(["CUDA", "cuDNN"])
    using CUDA, cuDNN
end
...
function clear_old_flowception_outputs()
    for name in readdir(@__DIR__)
        occursin(r"^flowception_demo_.*\.(pdf|png|mp4)$", name) || continue
        rm(joinpath(@__DIR__, name); force = true)
    end
end

clear_old_flowception_outputs()
...
struct ToyFlowception{L}
    layers::L
end

Flux.@layer ToyFlowception

function ToyFlowception(dim, depth)
    nheads = 8
    head_dim = dim ÷ nheads
    layers = (;
        ...
        insertion_decoder = Dense(dim => 1),
    )
    return ToyFlowception(layers)
end

function (m::ToyFlowception)(t, X::FlowceptionState)
    Xt = X.state
    l = m.layers
    locs = tensor(Xt[1])
    disc = tensor(Xt[2])
    local_t = reshape(X.local_t, 1, size(X.local_t)...)
    global_t = zero(local_t) .+ reshape(t, 1, 1, :)
    ...
    return (l.loc_decoder(x), l.disc_decoder(x)), l.insertion_decoder(x)[1, :, :]
end

function make_source(P::FlowceptionFlow; nstart = 1, T = Float32)
...
P = FlowceptionFlow(
    ...
    total_time = parse(Float32, get(ENV, "FLOWCEPTION_DEMO_TOTAL_TIME", "2")),
)

model_dim = parse(Int, get(ENV, "FLOWCEPTION_DEMO_DIM", "256"))
model_depth = parse(Int, get(ENV, "FLOWCEPTION_DEMO_DEPTH", "8"))
model = ToyFlowception(model_dim, model_depth) |> dev
...
orig_eta = eta = parse(Float64, get(ENV, "FLOWCEPTION_DEMO_LR", "0.005"))
opt_state = Flux.setup(Muon(eta = orig_eta), model)

iters = parse(Int, get(ENV, "FLOWCEPTION_DEMO_ITERS", "2000"))
batchsize = parse(Int, get(ENV, "FLOWCEPTION_DEMO_BATCH", "64"))
warmdown_steps = parse(Int, get(ENV, "FLOWCEPTION_DEMO_WARMDOWN_STEPS", "1000"))
warmdown_start = max(iters - warmdown_steps, 0)

for i in 1:iters
    ts = flowception_bridge(P, [X1target() for _ in 1:batchsize], Uniform(0f0, P.total_time), nstart = 1) |> dev
    loss, (∇model,) = Flux.withgradient(model) do m
        ...
        insertion_loss = floss(P, hat_insertions, ts.insertions_target, ts.Xt.padmask .* ts.Xt.branchmask, scalefloss(P, global_t, 1, 0.2f0))
        if i % 25 == 0
...
nsamples = parse(Int, get(ENV, "FLOWCEPTION_DEMO_NSAMPLES", "5"))
sample_dt = parse(Float32, get(ENV, "FLOWCEPTION_DEMO_SAMPLE_DT", "0.002"))
animation_max_frames = parse(Int, get(ENV, "FLOWCEPTION_DEMO_ANIMATION_MAX_FRAMES", "300"))
animation_fps = parse(Int, get(ENV, "FLOWCEPTION_DEMO_ANIMATION_FPS", "20"))
...
for sample_ix in 1:nsamples
    ...
    outfile = joinpath(@__DIR__, "flowception_demo_sample_$(sample_ix).pdf")
    savefig(pl, outfile)
    ...
    mp4file = joinpath(@__DIR__, "flowception_demo_sample_$(sample_ix).mp4")
