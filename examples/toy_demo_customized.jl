...
function X1target()
    ...
    #X1 = fixedcount_del_insertions(X1, 100) #Maximum deletions!
...
P = CoalescentFlow((BrownianMotion(0.01f0), Flowfusion.DistNoisyInterpolatingDiscreteFlow()), Beta(1,1), SequentialUniform()) #Good for viz

P = CoalescentFlow((OUBridgeExpVar(5f0, 10f0, 0.001f0, dec = -1f0), Flowfusion.DistNoisyInterpolatingDiscreteFlow()), Beta(1,1), SequentialUniform()) #Good for viz
...
pl = plot(stack(traj)')
savefig(pl, "../examples/testP_$(P.P[1]).pdf")
...
#P = CoalescentFlow((BrownianMotion(0.01f0), Flowfusion.DistNoisyInterpolatingDiscreteFlow()), Beta(1,1), SequentialUniform(), Beta(1,10)) #Good for viz


#You can overload these for specific behavior.
function BranchingFlows.canonical_anchor_merge(S1::DiscreteState{<:AbstractArray{<:Signed}}, S2::DiscreteState{<:AbstractArray{<:Signed}}, w1, w2) #Will dispatch on non-onehot
    dest = copy(S1)
    if rand() < w1 / (w1 + w2)
        dest.state .= S1.state
    else
...
P = CoalescentFlow((BrownianMotion(0.005f0), Flowfusion.DistNoisyInterpolatingDiscreteFlow()), Beta(1,1), BranchingFlows.del_weighted_sequential(w_tt=1.0, w_tf=100.0, w_ff=0.001), Beta(1,2.5)) #Good for viz
#P = CoalescentFlow((BrownianMotion(0.005f0), Flowfusion.DistNoisyInterpolatingDiscreteFlow()), Beta(1,1), BranchingFlows.tree_ordered_proximity(), Beta(1,1)) #Good for viz


t = Uniform(0f0,1f0)
batch_size = 500
@time bat = branching_bridge(P, X0sampler, [X1target() for _ in 1:batch_size], t, merger = BranchingFlows.canonical_anchor_merge, coalescence_factor = 1.0, use_branching_time_prob = 0.0);
pl = scatter(bat.t, sum(bat.Xt.padmask, dims = 1)[:], msw = 0, color = "black", alpha = 0.1)
savefig(pl, "../examples/counts2.pdf")

bat.X1anchor[2].S.state
...
for _ in 1:15
    ...
    pl
    savefig(pl, "../examples/treeordered_$(P.P[1])_$(P.coalescence_policy)_$(rand(1000001:9999999)).pdf")
end
