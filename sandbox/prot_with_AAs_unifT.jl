...
rundir = "BranchChainV1_coalfac0.2_unifT"
mkpath(rundir)
...
#=
function upper_length_bound(length, max_merges, branch_time_dist, t; upper_quantile = 0.9, coalescence_factor = 1.0)
    return quantile(Binomial(max_merges, cdf(branch_time_dist,t)), upper_quantile) + (length - max_merges)
end

function ind_and_upper_lengths(dat, branch_time_dist, draw_times; l2b = length2batch(1000, 1.9), max_length = 2000, coalescence_factor = 1.0)    
    upper_lengths = [upper_length_bound(length(d.chainids), length(d.chainids) - length(unique(d.chainids)), branch_time_dist, draw_times[i], coalescence_factor = coalescence_factor) for (i,d) in enumerate(dat)]
...
function (fc::BranchChainV1)(t, Xt, chainids, resinds; sc_frames = nothing, sc_AAs = nothing, sc_splits = nothing, sc_condmask = nothing)
    ...
    #For some reason this was causing crazy loss spikes (that seemed to concentrate in the discrete components) for time->1, and NaNs. Haven't figured out why.
    #I guess when time->1 some discrete "logits" shoot up, and this was getting amplified in the inputs, and time wasn't sampled near 1 often enough to learn to reduce them?
    #The other option is if there is some dimensional problem with this line, and its actually accumulating over a lot of dims or something.
    #Maybe using exp instead of softplus for the rate transforms would help here, because the logits wouldn't have to spike so high?
    #Another option would be pushing them through a norm or something.
...
P = CoalescentFlow((BrownianMotion(0.1f0), ManifoldProcess(0.1f0), DoobMatchingFlow(UniformDiscrete(1f0), true, x -> Flowfusion.NNlib.softplus(x) .+ 1f-8)), Beta(5,6))
...
function losses(P, X1hat, loss_bundle)
    ...
    l_splits = floss(P, hat_splits, loss_bundle.splits_target, loss_bundle.padmask, scalefloss(P,loss_bundle.t,1, 0.2f0)) / 1
...
#sched = burnin_learning_schedule(0.000005f0, 0.002f0, 1.05f0, 0.999975f0)
...
mpath = "/home/murrellb/ProtModels/BranchingFlows.jl/sandbox/BranchChainV1_coalfac0.1/model_epoch_2.jld"
Flux.loadmodel!(model, JLD2.load(mpath, "model_state"))
model = model |> device
opt_state = JLD2.load(mpath, "opt_state") |> device
sched = burnin_learning_schedule(0.001f0, 0.002f0, 1.15f0, 0.9995f0)
...
function mod_wrapper(t, Xₜ)
    ...
    #num_resolved_AAs = sum(tensor(Xtstate[3]) .!= 21)
    ...
    sc_condmask = trues(size(resinds)) |> device
    sc_frames, sc_aas, sc_splits = model(input_bundle..., sc_frames = sc_frames, sc_AAs = sc_aas, sc_splits = sc_splits, sc_condmask = sc_condmask)
    sc_frames, sc_aas, sc_splits = model(input_bundle..., sc_frames = sc_frames, sc_AAs = sc_aas, sc_splits = sc_splits, sc_condmask = sc_condmask)
    pred = model(input_bundle..., sc_frames = sc_frames, sc_AAs = sc_aas, sc_splits = sc_splits, sc_condmask = sc_condmask) |> cpu
    #@show pred[3]
    #if t < 0.1f0
    #    pred[3] .= min.(1f0, pred[3])
    #end
...
function test_sample(path; numchains = 1, chainlength_dist = Poisson(30))
    ...
    X0 = BranchingFlows.BranchingState((X0.state[1], X0.state[2], onehot(X0.state[3])), groupings) #Converts to onehot
...
for epoch in 3:100
    ...
    #    sched = burnin_learning_schedule(0.00001f0, 0.002f0, 1.05f0, 0.999975f0)
    ...
    #@time sampled_inds, upper_lengths = ind_and_upper_lengths(dat, P.branch_time_dist, draw_times, l2b = length2batch(1500, 1.9), coalescence_factor = 0.1);
    ...
    for (i, b) in enumerate(batchinds)
        ...
        #bat = branching_bridge(P, X0sampler, X1s, draw_times[b], maxlen = maximum(upper_lengths[b]), coalescence_factor = 0.1);
        #times = [Float32(rand(Beta(1,2))) for _ in 1:length(b)]
        times = rand(Float32, length(b))
        bat = branching_bridge(P, X0sampler, X1s, times, coalescence_factor = 0.2);
        ...
        print("Batch $i, dim: $(size(groupings)), t: $(times), scflag: $(scflag), ")
        l, grad = Flux.withgradient(model) do m
            ...
            println("l_loc: $l_loc, l_rot: $l_rot, l_aas: $l_aas, l_spl: $l_splits, l_tot: $(l_loc + l_rot + l_aas + l_splits)")
            l_loc + l_rot + l_aas + l_splits
        end
        if !isfinite(l)
            println("Non-finite loss, skipping batch")
            continue
        end
        #print("$(size(groupings)):$l, ")
        avg_l += l
