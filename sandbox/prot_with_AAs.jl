...
rundir = "BranchChainV1_coalfac0.1"
mkpath(rundir)
...
for epoch in 1:100
    ...
    for (i, b) in enumerate(batchinds)
        ...
        print("Batch $i, dim: $(size(groupings)), t: $(times), ")
    ...
    if epoch > 1
        for samp in 1:10
            test_sample("$(rundir)/test_sample_$(epoch)_$(samp).pdb", numchains = rand(1:3), chainlength_dist = Poisson(rand(10:150)))
        end
    end
